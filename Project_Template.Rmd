...
Energy_investment2021 %>% 
  na.omit()

Energy_investment2021$TotalFedAction_EnergyEnv_2021 <-
  as.numeric((gsub(",","",Energy_investment2021$TotalFedAction_EnergyEnv_2021)))
...
```{r}
model <- lm(data = census2021clean, Energy_investment ~ `Total population` + Female + child + `Median age (years)` + `Median household income (dollars)` + `Occupied housing units` + Gas + Electricity + `All other fuels` + `No fuel used`)
summary(model)
```
