...
Gas | % | % of gas used in household
Electricity | % |% of electricity used in household
State |- | -
Year |- |-
...
As part of our exploration, we were interested in where the most disasters in the United States are occurring. If climate disasters influence investment behavior, we might expect states with the highest frequency of disasters to also have the greatest federal investments in the energy and environment sectors.
...
We were also interested in the states that received the most federal grants and loans towards energy and environment.
...
States with the highest ratio of spending towards energy and environment programs are DC, Wyonming, South Dakota, and Montana. 
...
p_ts <- ggplot(df_plot, aes(x = Year, y = Spending)) +
  ...
  labs(x = "Year", y = "National Energy & Environment Spending",
       title = "Annual National Investment (Energy & Environment): observed + linear trend") +
  theme_minimal()
print(p_ts)
...
summary(model_2) #frequency of disaster is related
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
model_3 <- lm(data = log_combined2021, log_EE_investment ~  `2021`, na.action = na.exclude)
summary(model_3) # Public awareness is related
```
