...
model <- lm(data = combined2021, Total_EE_Investment ~ `Total population` + Female + child + `Median age (years)` + `Median household income (dollars)` + `Occupied housing units` + Gas + Electricity + `All other fuels` + Disasters + `2021`)
summary(model)

model_2 <- lm(data = combined2021, Total_EE_Investment ~ `Total population` +  child + `Median age (years)` + Gas + Electricity + `All other fuels`)
summary(model_2)
step(model_2)

model_AIC<- lm(data = combined2021, Total_EE_Investment ~ `Total population` + Female + child + `Median age (years)` + `Median household income (dollars)` + `Occupied housing units` + Gas + Electricity + `All other fuels` + Disasters+ `2021`, na.action = na.exclude)
step(model_AIC, direction = "both")
...
model_4 <- lm(data = combined2021, `2021` ~ Total_EE_Investment+ `Total population` + Female + child + `Median age (years)` + `Median household income (dollars)` + `Occupied housing units` + Gas + Electricity + `All other fuels` + Disasters,na.action = na.exclude)
step(model_4)
summary(model_4)

model_5 <- lm(data = combined2021, Disasters ~  Total_EE_Investment)
summary(model_5)
#AIC?
```
